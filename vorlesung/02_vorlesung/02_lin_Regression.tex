...
\label{eq:Kap2ModellglmitResiduenlinearReg}
\end{equation}
...
\label{eq:Kap2KostenfktlinearReg}
\end{equation}
...
\label{eq:partielleAblRegressoren}
\end{equation}
Somit erhalten wir:
\begin{equation}
...
Als nächstes ermitteln wir, um das \textbf{vollständige	Messergebnis} zu erhalten,
auch die Schätz\-werte der Kovarianzen. Die Hauptdiagonale der Kovarianzmatrix sind die Varianzen.
\begin{equation}
...
\label{eq:COVRegressionsparameter}
\end{equation}
...
\begin{equation}
s_{\mathrm{M}}^2 = \frac{1}{J - 1}\sum\limits_{j = 1}^J {(\hat{\theta}_0 + \hat{\theta}_1 X_j - \bar
	{Y})^2}
\end{equation}
...
\label{eq:totEmpVarianz}
\end{equation}
Beweis von Gl.(\ref{eq:totEmpVarianz}): 
\begin{align}
	s_T^2 & = \frac{1}{J-1}\sum\limits_{j = 1}^J {(Y_j - \bar {Y})^2} \\
		&  =  \frac{1}{J-1}\sum\limits_{j = 1}^J {\left[(Y_j-\hat Y_j)+(\hat{Y_j}-\bar{Y})       \right]^2}\\ 
		& = \frac{1}{J-1}\left[\sum\limits_{j = 1}^J (Y_j-\hat Y_j)^2 + \sum\limits_{j = 1}^J (\hat Y_j-\bar Y_j)^2 +2 \cdot \sum\limits_{j = 1}^J (Y_j-\hat Y_j)(\hat Y_j - \bar Y)  \right]  \\
		& = s_R^2 + s_M^2 + \frac{1}{J-1} \cdot 2 \cdot \sum\limits_{j = 1}^J (Y_j-\hat Y_j)(\hat Y_j - \bar Y)
\end{align}
Es ist jetzt noch zu zeigen, dass
\begin{equation}
	\sum\limits_{j = 1}^J (Y_j-\hat Y_j)(\hat Y_j - \bar Y) \overset{!}{=} 0
\end{equation}
Für Punkte $\hat Y_j$ auf der Geraden gilt: 
\begin{equation}
	\hat Y_j = \hat \theta_0 + \hat \theta_1 X_j
\end{equation}
Der y-Abschnitt $\hat \theta_0$ bei der Regressionsgeraden kann  wie folgt berechnet werden (Hinweis: Schwerpunkt $(\bar X | \bar Y)$ liegt auf der Regressionsgeraden): 
\begin{equation}
	\hat \theta_0 = \bar Y - \hat \theta_1 \bar X
\end{equation}
Daraus folgt: 
\begin{equation}
	\hat Y_j = \bar Y - \hat \theta_1 \bar X + \hat \theta_1 X_j
\end{equation}
Diese Gleichung etwas umstellen und sowohl links als auch rechts $Y_j$ addieren, ergibt: 
\begin{equation}
 Y_j - \hat Y_j = (Y_j - \bar Y)- \hat \theta_1(X_j-\bar X)
 \label{eq:1_Beweis_totale_empirische_Varianz}
\end{equation}
Zudem gilt: 
\begin{equation}
	(\hat Y_j- \bar Y) = \hat \theta_1 (X_j-\bar X)
	 \label{eq:2_Beweis_totale_empirische_Varianz}
\end{equation}
Gl.(\ref{eq:1_Beweis_totale_empirische_Varianz}) und Gl.(\ref{eq:2_Beweis_totale_empirische_Varianz}) eingesetzt, ergibt: 
\begin{align}
\sum\limits_{j = 1}^J (Y_j-\hat Y_j)(\hat Y_j - \bar Y) & = 
\hat \theta_1 \sum\limits_{j = 1}^J  \left[\left((Y_j-\bar Y)- \hat \theta_1 (X_j-\bar X)\right) (X_j - \bar X)\right] \\
&= \hat \theta_1 \left[ \sum\limits_{j = 1}^J (Y_j-\bar Y)(X_j-\bar X) - \hat \theta_1 \sum\limits_{j = 1}^J  (X_j-\bar X)^2  \right]
& = 0
\end{align}
Da für das hintere $\hat \theta_1$ kann folgendes eingesetzt werden: 
\[
\hat \theta_1 = \frac{s_{XY}}{s_X^2} = \frac{\sum\limits_{j = 1}^J (X_j-\bar X)(Y_j - \bar Y)}{\sum\limits_{j = 1}^J (X_j-\bar X)^2}
\]
Damit wäre Gl.(\ref{eq:totEmpVarianz}) bewiesen. \\
% \nonumber

Das \textbf{Bestimmtheitsma{\ss}} ist definiert als das Verh\"{a}ltnis von
...
\[
\boldsymbol\theta = \left(\theta_0, \theta_1, \theta_2,\theta_3 \right)^T
\]
Die optimalen Parameter $\boldsymbol{\hat \theta}$ ergeben sich durch Lösen
des folgenden Gleichungssystems, siehe Gl.(\ref{eq_Bestimmung_von_hat_theta})
\[
