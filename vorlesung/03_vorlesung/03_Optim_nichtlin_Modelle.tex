...
Eine Schätzung der Modellparameter erfolgt durch Variation, also durch Optimierung derart, dass die 
Abweichungen des Modells von beobachteten Werten einer Messung direkt zugänglicher Größen minimal werden.
Dazu haben wir eine Optimierungszielfunktion $Q$, \textsl{Zielfunktional} oder \textsl{Kostenfunktion}
genannt, definiert. Wir sind von normalverteilten Abweichungen ausgegangen und haben uns
...
Abb.~\ref{LSoptiExample1} zeigt das Modell als schwarz gestrichelte Gerade und die Beobachtungen als blaue Kreise, rechts die Likelihoodverteilung. Wir hatten bereits ausführlich darüber gesprochen, dass die Suche des Maximums der Likelihood in die Suche des Minimums übergeht. Es gelte nun diejenige Geradenrichtung zu finden, für die die senkrechten Abstände,
die Residuen $\varepsilon_j$,  der beobachteten Punkte (Messpunkte) $\mathbf{r}_{\mathrm{Mess},j} = (X_{1,j}, X_{2,j}, X_{3,j})^\mathsf{T}$ minimal sind. Für das Kriterium für minimale Abstände setzen wir wieder voraus, dass die Messpunkte gaußverteilt um eine Gerade streuen. Den Betrag für die senkrechten Abstände berechnen wir mit
\begin{equation}
...
\end{equation}
Für den Schätzvorgang machen wir einen Optimierungsvorgang gemäß der Methode der kleinsten
...
Das Funktional, das es zu Minimieren gilt, wird auch \textsl{Zielfunktional} oder \textsl{Kostenfunktion}
genannt, allgemein werden wir es mit dem Symbol $Q$ bezeichnet.
...
in Matlab und Gnu-Octave zur Verfügung gestellt. Mit Simplex ist ein Vieleck gemeint, das $M+1$ hat für $M$
Parameter Ecken, in unserem Beispiel also drei Ecken für zwei Parameter, was in Abb.~\ref{LSoptiExample1NM} durch
schwarz umrandete weiße Punkte, die mit gestrichelten Linien verbunden sind, skizziert ist. Zum Aufbau eines
...
streuen, sondern stark abweichende Punkte aufweisen, greift man auf andere Verteilungen zu.
...
\end{equation}
bildet sich die Minimumsmulde durch die Betragsbildung zu einer scharfen Spitze ausprägt.
...
den rundlichen, stetig differenzierbaren Residuenquadratsummen haben, können wir Verfahren
einsetzen, die nicht nur die RQS direkt, sondern auch deren Gradienten verwenden.
...
Abhangs der RQS-Funktion zusammen mit dem Wert der Steigung, an der Position des Startwerttupels
bestimmt. Dieses Verfahren beleuchten wir im folgenden Abschnitt anhand unseres Beispiels ein wenig genauer.
...
so dass sich entsprechend die Kostenfunktion $Q$ Funktion des Parametervektors ist
\begin{equation}
...
Dies erfolgt solange, bis wir beliebig dicht an dem Minimum von $Q$ gelangt sind. Es gibt damit eine
...
Die einzelnen Iterationsschritte $\kappa$ bedeuten eine Revsion des Modellparametervektors
\begin{equation}
...
\label{TaylorResi1}
\end{equation}
Hier bilden die partiellen Ableitungen der einzelnen Residuen als $J \times M$-Jakobimatrix
\begin{equation}
...
Die Jacobimatrix ist ein $J \times M$-Matrix. Der Begriff Jacobimatrix bedeutet nur, dass es
...
der einzelnen Residuen (anders als vorige Woche für die Vektorkomonenten als Projektionen auf die Achsen der direkten
Messgrößen).
\begin{equation}
...
\end{equation}
Die Lösung dieses linearen Gleichungssystems liefert und den Inkrementvektor $\Delta \mathbf{p}_\kappa$
...
dem als Störung zum einen Signal mit höherer Frequenz und kleinerer Amplitude aufmoduliert ist
...
durch Einführen eines Dämpfungsterms, der als Schrittweitenfaktor den Inkrementvektor
...
\end{equation}
Damit hatten wir im vorigen Kapitel einen Ansatz erhalten, der
...
Dämpfungsparameter $\mu$ zu wählen, ist heuristisch.
...
Die Varianzen (und Kovarianzen) der Residuen sind immer auch Funktion der Modellparameter, sie
lassen sich nur bei den in der heutigen Vorlesung betrachteten Beispielen ausklammern, also
\begin{equation}
Q(\mathbf{p}) \; = \;
...
Für den Fall, dass es eine Varianz für ein Residuum eines Beobachtungstupels
$(X_{1,j},\dots,X_{N,j})$ gibt, die Varianz also eine skalare Größe hinsichtlich
des Raumes der direkten Messgrößen ist, können wir $\sqrt{\sigma_j^2} = \sigma_j$ 
den Residuen zuordnen mit
\begin{equation}
Q(\mathbf{p}) \; = \;
...
In der vorigen Vorlesung hatten wir angesprochen, dass es aber auch Fragestellungen gibt,
für die es sein kann, dass die unterschiedlichen
direkten Messgrößen $X_i$ unterschiedliche Varianzen $\sigma_i^2$ aufweisen. Ein Beispiel dafür
war eine Gerade in einer Ebene mit den Modellparametern $\alpha$ für den Winkel und
$d$ für den Abstand der Geraden vom Koordinatenursprung
\begin{equation}
Q(\alpha, d) \; = \;
 \left(\frac{\varepsilon_j(\alpha, d) \sin(\alpha)}{\sigma_1}\right)^2
\; + \; \left(\frac{\varepsilon_j(\alpha, d) \cos(\alpha)}{\sigma_2}\right)^2 .
\end{equation}
