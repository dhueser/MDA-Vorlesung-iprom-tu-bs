...
ist die Simplexmethode nach Nelder und Mead aus dem Jahr 1965. Er wird als Bibliotheksfunktion \texttt{fminsearch} in Gnu-Octave oder 
...
% Stand: 2019-11-05
% Laeuft unter Matlab und Octave
t = [0:99]'; % Sekunden
N = length(t);
a = 5; % Volt
lam = 30; % Sekunden
...
function fit_sin(gen_new)
 % Stand: 2019-11-05
 % Laeuft nur unter Octave, da leasqr nur unter Octave
 % frei zur Verfuegung steht. Bei Matlab gibt es
 % einen aehnlichen Befehl, der heisst nlinfit
  load('pltSS_nonlin_leastsquare_sin_zB2.mat');
  p0 = [34; 0.23*pi];
  sf = @(x, p) a*sin(2*pi*x/p(1) + p(2));
